...
class HarukaClient extends Client {
    ...
    async start() {
        ...
        if (!process.env.TOKEN) {
            logger.error('No TOKEN found in environment. Aborting start.');
        ...
        try {
            ...
            logger.warn('No statsVoiceUpdater or failed to start it: ' + (e.message || e));
        }
        logger.info('HarukaClient started');
    }
}
