...
module.exports = {
    ...
    async execute(message, args, client) {
        try {
            ...
            const dbLevels = db.getPermissionLevels(message.guild.id);
            ...
            if (dbLevels.length > 0) {
                dbLevels.forEach(row => {
                    if (permissionLevels[row.level]) {
                        permissionLevels[row.level].push(row.role_id);
                    }
                });
            } else {
                // Fallback sur les IDs par défaut si la DB est vide pour ce serveur
                Object.assign(permissionLevels, {
